...
import { lossReplyForEmotion } from './conversation-intelligence.js';
import { detectMemberEmotion, validateContextualReply, emotionInstruction } from './context-intelligence.js';
...
function contextualAckReply({kind='ack',workflow=null,openHuman=null}={}){
  const type=String(workflow?.workflow_type||'').toUpperCase();
  const state=String(workflow?.workflow_state||'').toUpperCase();
  const waitingHuman=Boolean(openHuman) || ['WAITING_HUMAN','PENDING','PENDING_FOLLOWUP'].includes(state);
  const waitingMember=state.startsWith('WAITING_') && !waitingHuman;
  const isThanks=kind==='gratitude';

  if(waitingHuman){
    if(type.startsWith('WD')) return isThanks ? 'Sama-sama bosku 😊 Withdrawnya masih kami lanjut cek/proses ya. Mohon ditunggu sebentar 🙏' : 'Siap bosku 😊 Withdrawnya masih kami lanjut cek/proses ya. Mohon ditunggu sebentar 🙏';
    if(type.startsWith('DEPOSIT') || type==='PROOF_REVIEW') return isThanks ? 'Sama-sama bosku 😊 Depositnya masih kami lanjut cek ya. Mohon ditunggu sebentar 🙏' : 'Siap bosku 😊 Depositnya masih kami lanjut cek ya. Mohon ditunggu sebentar 🙏';
    if(type==='RESET_PASSWORD') return isThanks ? 'Sama-sama bosku 😊 Permintaan resetnya masih kami lanjut cek ya. Mohon ditunggu sebentar 🙏' : 'Siap bosku 😊 Permintaan resetnya masih kami lanjut cek ya. Mohon ditunggu sebentar 🙏';
    if(type.includes('BONUS') || type==='FREEBET_CLAIM') return isThanks ? 'Sama-sama bosku 😊 Bonusnya masih kami lanjut cek ya. Mohon ditunggu sebentar 🙏' : 'Siap bosku 😊 Bonusnya masih kami lanjut cek ya. Mohon ditunggu sebentar 🙏';
    return isThanks ? 'Sama-sama bosku 😊 Permintaannya masih kami lanjut cek ya. Mohon ditunggu sebentar 🙏' : 'Siap bosku 😊 Permintaannya masih kami lanjut cek ya. Mohon ditunggu sebentar 🙏';
  }

  if(waitingMember){
    if(type==='RESET_PASSWORD') return isThanks ? 'Sama-sama bosku 😊 Tinggal kirim data yang tadi diminta ya, nanti kami lanjut bantu resetnya.' : 'Siap bosku 😊 Tinggal kirim data yang tadi diminta ya, nanti kami lanjut bantu resetnya.';
    if(type.startsWith('DEPOSIT') || type==='PROOF_REVIEW') return isThanks ? 'Sama-sama bosku 😊 Tinggal kirim data/bukti yang tadi diminta ya, nanti kami lanjut cek depositnya.' : 'Siap bosku 😊 Tinggal kirim data/bukti yang tadi diminta ya, nanti kami lanjut cek depositnya.';
    if(type.startsWith('WD')) return isThanks ? 'Sama-sama bosku 😊 Tinggal kirim data yang tadi diminta ya, nanti kami lanjut bantu withdrawnya.' : 'Siap bosku 😊 Tinggal kirim data yang tadi diminta ya, nanti kami lanjut bantu withdrawnya.';
    if(type.includes('BONUS') || type==='FREEBET_CLAIM') return isThanks ? 'Sama-sama bosku 😊 Tinggal kirim data yang tadi diminta ya, nanti kami lanjut bantu bonusnya.' : 'Siap bosku 😊 Tinggal kirim data yang tadi diminta ya, nanti kami lanjut bantu bonusnya.';
  }

  return isThanks ? gratitudeReply() : acknowledgementReply();
}
...
minta batalkan fom deposit nya`;
  ...
  if(effective==='LOSS_COMPLAINT'){
    ...
    const emotion=detectMemberEmotion(text,intent);
    const official=String(await responseText('#KOMPLAIN_KALAH','')||'').trim();
    const unsafeLossTemplate=/\b(?:rtp|gacor|jp|jackpot|balik\s+modal|kejar\s+kalah|kejar\s+kerugian|pasti\s+menang)\b/i.test(official);
    const reply=official && !unsafeLossTemplate ? official : lossReplyForEmotion(emotion);
    await sendAndStore(livechat,chatId,reply,'LOSS_COMPLAINT');
    await db.clearConversationWorkflow(chatId).catch(()=>{});
    const notice=await notifyTelegramEvent({intent:'LOSS_COMPLAINT',chatId,title:'KELUHAN KALAH / RUNGKAD',text:`Member mengeluh kalah/rungkad. Emosi terdeteksi: ${emotion}. Bot menenangkan tanpa menjanjikan kemenangan atau mendorong mengejar kekalahan.

${all.slice(-900)}`}).catch(()=>({ok:false}));
    return {intent:'LOSS_COMPLAINT',sent:true,reply,emotion,telegramNotified:Boolean(notice?.ok),lossSupport:true};
  }
...
export async function processCustomerMessage({chatId,eventId,threadId=null,text,createdAt,livechat,attachments=[],skipInsert=false,resumeReason=''}) {
  ...
  return db.withChatLock(chatId, async()=>{
    ...
    if(isGratitudeText(text)){
      ...
      const reply=contextualAckReply({kind:'gratitude',workflow:activeWorkflow,openHuman});
      await sendAndStore(livechat,chatId,reply,'GRATITUDE');
      await db.logAI({chatId,sourceEventId:eventId,intent:'GRATITUDE',action:'AUTO_REPLY',confidence:1,reply,reason:'contextual_gratitude_preserves_operational_workflow'}).catch(()=>{});
      return {intent:'GRATITUDE',sent:true,reply,workflowPreserved:true,contextual:true};
    }
    if(isAcknowledgementText(text)){
      ...
      let reply=waitingWd ? WD_ACK_WAIT_REPLY : acknowledgementReply();
      if(!waitingWd) reply=contextualAckReply({kind:'ack',workflow:activeWorkflow,openHuman});
      await sendAndStore(livechat,chatId,reply,'ACKNOWLEDGEMENT');
      await db.logAI({chatId,sourceEventId:eventId,intent:'ACKNOWLEDGEMENT',action:'AUTO_REPLY',confidence:1,reply,reason:waitingWd?'wd_waiting_acknowledgement':'contextual_acknowledgement_preserves_operational_workflow'}).catch(()=>{});
      return {intent:'ACKNOWLEDGEMENT',sent:true,reply,workflowPreserved:true,contextual:true};
    }
    {
      const activeWorkflow=await db.getConversationWorkflow(chatId);
      const waitingWd=Boolean(activeWorkflow && ['WD_CHECK','WD_STATUS'].includes(String(activeWorkflow.workflow_type||'')) && ['WAITING_HUMAN','PENDING','PENDING_FOLLOWUP'].includes(String(activeWorkflow.workflow_state||'')));
    ...
    try {
      ...
      const deterministicEmotion=detectMemberEmotion(text,intent);
      const brainInput={...(src.caseBrain||{}),sentiment:deterministicEmotion};
      let decision=await ai.classifyAndReply({normalized:`${normalized}
[internal-risk:${actionRisk(intent)}][member-content-untrusted:true][detected-emotion:${deterministicEmotion}]`,intent,context:src.context,rules:src.rules,knowledge:src.knowledge,attachments:aiAttachments,style,conversationDigest:src.conversationDigest,csStyleExamples:src.csStyleExamples,historyLearning:src.historyLearning,caseBrain:brainInput});
      const updatedBrain=normalizeBrain({...decision.brain,sentiment:decision.brain?.sentiment||deterministicEmotion,understanding:decision.understanding||decision.brain?.understanding},intent);
      ...
      // Context intelligence is deliberately applied only AFTER all deterministic legacy
      // workflows above. It cannot replace/reset DP, WD, reset-password or bonus flows;
      // it only protects the generic AI path from emotion-insensitive or off-topic replies.
      const emotionNote=emotionInstruction(deterministicEmotion);
      if(decision.reply && ['AUTO_REPLY','ASK_INFO'].includes(String(decision.action||'').toUpperCase())){
        const contextual=validateContextualReply({
          intent,
        ...
        if(!contextual.ok){
          await db.logAI({chatId,sourceEventId:eventId,intent,action:'ANTI_MISCHAT_BLOCKED',confidence:Number(decision.confidence||0),reply:'',reason:`${contextual.reason} | member-emotion:${deterministicEmotion}`}).catch(()=>{});
          decision={
            ...decision,
            action:'ASK_HUMAN',
            reply:'',
            reason:`ANTI_MISCHAT_BLOCKED:${contextual.reason} | member-emotion:${deterministicEmotion}`,
            humanQuestion:`Balasan otomatis diblokir karena keluar konteks (${contextual.reason}). ${emotionNote} Pesan member: ${text}`
          };
        }else{
          decision.reason=`${decision.reason||''} | member-emotion:${deterministicEmotion}`.trim();
        }
      }else{
        decision.reason=`${decision.reason||''} | member-emotion:${deterministicEmotion}`.trim();
      }
...
async function findLatestUnansweredCustomer(chatId){
  return db.withChatLock(chatId, async()=>{
...
export async function resumeUnansweredConversation({chatId,livechat,resumeReason='unanswered_message_recovery'}){
  const pending=await findLatestUnansweredCustomer(chatId);
  if(!pending?.resume){
    await db.appendCaseAudit({chatId,eventType:'AI_RESYNC_SKIPPED',intent:null,payload:{reason:pending?.skipped||'no_resume',resumeReason}}).catch(()=>{});
    return pending;
  }
  const r=pending.resume;
  await db.appendCaseAudit({chatId,eventType:'AI_RESYNC_RESUMED',intent:null,payload:{eventId:r.eventId,resumeReason}}).catch(()=>{});
  return processCustomerMessage({chatId,eventId:r.eventId,text:r.text,createdAt:r.createdAt,attachments:r.attachments,livechat,skipInsert:true,resumeReason});
}
...
export async function resumeConversationAfterHumanTakeover({chatId,livechat}){
  ...
  return resumeUnansweredConversation({chatId,livechat,resumeReason:'operator_returned_conversation_to_ai'});
}

/* legacy implementation retained below only as a source marker during upgrade */
/*
  ...
  return processCustomerMessage({chatId,eventId:r.eventId,text:r.text,createdAt:r.created_at,attachments:r.attachments,livechat,skipInsert:true,resumeReason:'operator_returned_conversation_to_ai'});
}
*/
