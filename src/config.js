import { isUsableTotpSecret } from './totp-secret.js';
...
export const config = {
  ...
  admin2faEnabled: bool(process.env.ADMIN_2FA_ENABLED, false),
  adminTotpSecret: process.env.ADMIN_TOTP_SECRET || '',
  adminSessionHours: Math.min(24, Math.max(1, int(process.env.ADMIN_SESSION_HOURS, 8))),
...
export function validateConfig() {
  ...
  if (!config.adminPassword && !config.adminPasswordHash) warnings.push('ADMIN_PASSWORD/ADMIN_PASSWORD_HASH belum diisi');
  if (!config.adminPasswordHash) warnings.push('Security: gunakan ADMIN_PASSWORD_HASH (scrypt) untuk production');
  if (config.admin2faEnabled && config.adminTotpSecret && !isUsableTotpSecret(config.adminTotpSecret)) warnings.push('ADMIN_TOTP_SECRET dari ENV tidak valid/placeholder; server akan memakai atau membuat secret 2FA terenkripsi di database');
  if (config.admin2faEnabled && !config.adminTotpSecret) warnings.push('ADMIN_TOTP_SECRET tidak diisi; server akan membuat secret 2FA otomatis dan menyimpannya terenkripsi di database');
  if (!config.admin2faEnabled) warnings.push('Security: ADMIN_2FA_ENABLED belum aktif');
...
export function assertBootConfig() {
  ...
  if (!config.adminPassword && !config.adminPasswordHash) throw new Error('CONFIG_ERROR: ADMIN_PASSWORD atau ADMIN_PASSWORD_HASH wajib diisi.');
