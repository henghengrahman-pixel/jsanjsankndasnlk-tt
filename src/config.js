...
export function validateConfig() {
  ...
  if (config.admin2faEnabled && !config.adminTotpSecret) warnings.push('ADMIN_2FA_ENABLED aktif tetapi ADMIN_TOTP_SECRET belum diisi');
...
export function assertBootConfig() {
  ...
  if (config.admin2faEnabled && String(config.adminTotpSecret||'').replace(/[^A-Z2-7]/ig,'').length < 16) throw new Error('CONFIG_ERROR: ADMIN_TOTP_SECRET tidak valid/minimal 16 karakter Base32.');
