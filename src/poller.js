...
import { processCustomerMessage, processGreetingTrigger, resumeUnansweredConversation } from './engine.js';
...
export async function syncOnce(livechat,{manual=false}={}){
  ...
  try {
    ...
    // Mirror LiveChat lifecycle immediately. Some Agent API responses keep an archived
    // summary around with is_followed=true; explicit inactive/closed states must still
    // disappear from the AI inbox in the same sync cycle.
    for(const summary of rawChats){
      if(!summary?.id) continue;
      const st=livechat.chatState(summary);
      if(st.terminal || st.active===false){
        await db.markConversationEnded(String(summary.id)).catch(()=>{});
    ...
    // No-silent recovery: a message may have been persisted while Auto Reply was OFF,
    // or a provider/API failure may have interrupted the reply after persistence.
    // Recover a bounded batch on every active polling cycle. Human takeover chats are
    // excluded by the DB query and re-checked inside the engine.
    let recovered=0, recoverySkipped=0;
    const autoReplyEnabled=Boolean(await db.getSetting('auto_reply',config.autoReplyDefault));
    if(autoReplyEnabled){
      const pendingChats=await db.listConversationsNeedingAIReply(20).catch(()=>[]);
      for(const row of pendingChats){
        const out=await resumeUnansweredConversation({chatId:String(row.chat_id),livechat,resumeReason:'poller_no_silent_recovery'}).catch(async e=>{
          await db.logError('poller','UNANSWERED_RECOVERY_FAILED',e.message,{chatId:String(row.chat_id)}).catch(()=>{});
          return {error:e.message};
        });
        if(out?.sent) recovered++; else recoverySkipped++;
      }
    }
    lastTick=new Date().toISOString();
    lastResult={ok:true,listSource:data?._listSource||'unknown',rawChats:rawChats.length,chats:chats.length,fetched,unchanged,fetchErrors,bootstrapped,newMessages,processed,recovered,recoverySkipped};
