...
let lastError=null,lastPollAt=null,lastHandledAt=null,lastBacklogScanAt=0,lastSlaScanAt=0,lastTransportCheckAt=0;
...
async function ackCallbackFast(tg,q,text=''){
  if(!tg||!q?.id) return false;
  let last=null;
  for(let i=0;i<3;i++){
    try{
      await tg.answerCallbackQuery(q.id,text);
      return true;
    }catch(e){
      last=e;
      if(i<2) await sleep(150*(i+1));
    }
  }
  lastError=last?.message||'CALLBACK_ACK_FAILED';
  await db.logError('human_bridge','CALLBACK_ACK_FAILED',lastError,{callbackId:String(q.id||''),userId:q.from?.id}).catch(()=>{});
  return false;
}

async function preAckCallbackBatch(updates,tg){
  const items=Array.isArray(updates)?updates:[];
  const callbacks=items.filter(u=>/^hb:\d+:[A-Z0-9_]+$/.test(String(u?.callback_query?.data||'')));
  await Promise.all(callbacks.map(async u=>{
    const q=u.callback_query;
    const ok=await ackCallbackFast(tg,q,'');
    if(ok) u.__bridgeCallbackAcked=true;
  }));
}

async function ensureTelegramPollingTransport(tg,{force=false}={}){
  if(!tg?.ready?.()) return {ok:false,reason:'invalid_token'};
  if(!force && Date.now()-lastTransportCheckAt<30000) return {ok:true,skipped:'fresh'};
  const info=await tg.getWebhookInfo().catch(e=>({__error:e}));
  if(info?.__error) throw info.__error;
  const webhookUrl=String(info?.url||'').trim();
  if(webhookUrl){
    await tg.deleteWebhook();
    const verify=await tg.getWebhookInfo().catch(()=>null);
    if(String(verify?.url||'').trim()) throw new Error('TELEGRAM_WEBHOOK_STILL_ACTIVE');
    await db.logError('human_bridge','WEBHOOK_REMOVED_FOR_POLLING','Webhook Telegram aktif ditemukan dan dihapus otomatis',{url:webhookUrl}).catch(()=>{});
  }
  lastTransportCheckAt=Date.now();
  return {ok:true,webhookRemoved:Boolean(webhookUrl)};
}
...
async function handleCallback(u,livechat){
  ...
  try{
    ...
    if(!authorizedTelegramUser(q.from)){
      if(!u.__bridgeCallbackAcked){
        try{ await tg.answerCallbackQuery(q.id,'Akun Telegram ini belum diizinkan.'); }
        catch{ await ackCallbackFast(tg,q,'Akun Telegram ini belum diizinkan.'); }
      }else{
        await tg.sendMessage(String(q.message?.chat?.id||''),'Akun Telegram ini belum diizinkan.',{replyTo:q.message?.message_id,topicId:q.message?.message_thread_id||null}).catch(()=>{});
      }
      return true;
    }
    // The batch pre-ACK path normally stops Telegram's spinner before this worker runs.
    // If pre-ACK failed (network blip), retry here before any DB/LiveChat work.
    if(!u.__bridgeCallbackAcked){
      try{ await tg.answerCallbackQuery(q.id,'Sedang diproses…'); }
      catch{ await ackCallbackFast(tg,q,'Sedang diproses…'); }
    }
  }catch(e){
    lastError=e.message;
  ...
  try{
    ...
    // Remove the keyboard only after the member reply and DB close both succeed.
    // This prevents accidental double clicks while preserving retry buttons on failure.
    await tg.editMessageReplyMarkup(String(q.message?.chat?.id||ticket.telegram_chat_id),q.message?.message_id,{inline_keyboard:[]}).catch(()=>{});
...
export async function startHumanBridge({livechat,onAnswer,onAction}){
  ...
  (async()=>{
    ...
    while(!stop){
      try{
        if(!processLock){
          processLock=await db.tryAcquireProcessLock('livechat_ai:telegram_bridge_poll');
          if(!processLock){
            lastError='TELEGRAM_BRIDGE_STANDBY: another app instance owns Telegram polling';
        ...
        await ensureTelegramPollingTransport(tg,{force:/conflict|webhook/i.test(String(lastError||''))});
        const updates=await tg.getUpdates({offset,timeout:20}); lastPollAt=new Date().toISOString();
        // Stop Telegram button spinners before any slower DB/LiveChat action handling.
        await preAckCallbackBatch(updates,tg);
    ...
    if(processLock) await processLock.release().catch(()=>{});
    running=false;
  })();
}
