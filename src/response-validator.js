...
import { replyTopicConflict, asksForbiddenForgotPasswordId } from './conversation-intelligence.js';
...
export function validateProDecision({decision={},history=[],brain={},intent='GENERAL',hasKnowledge=false}){
  ...
  // Anti-mischat: an autonomous reply may not silently jump to another operational topic.
  // Re-classification / human review is safer than sending a polished but wrong answer.
  if(reply && ['AUTO_REPLY','ASK_INFO'].includes(action)){
    const conflict=replyTopicConflict(intent,reply);
    if(conflict) return {...out,action:'ASK_HUMAN',reply:'',reason:`anti_mischat:${conflict}|${out.reason||''}`};
  }
  // Established reset-password workflow never asks the member for User ID.
  if(String(intent).toUpperCase()==='FORGOT_PASSWORD' && reply && asksForbiddenForgotPasswordId(reply)){
    return {...out,action:'ASK_HUMAN',reply:'',reason:`anti_mischat:forgot_password_user_id_forbidden|${out.reason||''}`};
  }
  ...
  const risky=new Set(['WITHDRAW_PROBLEM','DEPOSIT_PROBLEM','FORGOT_PASSWORD','BONUS','BONUS_REQUEST','BONUS_DAILY','ACCOUNT_CHANGE','ACCOUNT_CHANGE_REQUEST','PAYOUT_NOT_RECEIVED','BANK_ACCOUNT_LIMIT']);
