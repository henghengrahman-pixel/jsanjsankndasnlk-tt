...
app.post('/api/login',async(req,res)=>{
  ...
  const otpOk=passOk ? verifyTotp(otp) : false;
  if(!userOk||!passOk||!otpOk){
    ...
    if(config.admin2faEnabled && userOk && passOk && !String(otp||'').trim()) return res.status(401).json({ok:false,error:'OTP_REQUIRED',requires2fa:true});
...
app.get('/api/me',(req,res)=>{const token=getSessionToken(req);const session=verifySession(token,req);res.json({ok:Boolean(session),user:session?.u||null,twoFactorEnabled:config.admin2faEnabled,csrfToken:session?csrfTokenForSession(token):null,sessionExpiresAt:session?.exp||null});});
