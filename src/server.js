...
app.post('/api/login',async(req,res)=>{
  ...
  if(config.admin2faEnabled){
    ...
    if(!otpOk){
      ...
      return res.status(401).json({ok:false,error:'LOGIN_FAILED',requires2fa:true,setupRequired:!enrolled});
    }
    if(!enrolled){
      await setSetting('admin_2fa_enrolled',true);
...
app.get('/api/me',async(req,res)=>{const token=getSessionToken(req);const session=verifySession(token,req);const setupRequired=config.admin2faEnabled?!Boolean(await getSetting('admin_2fa_enrolled',false)):false;res.json({ok:Boolean(session),user:session?.u||null,twoFactorEnabled:config.admin2faEnabled,twoFactorSetupRequired:setupRequired,csrfToken:session?csrfTokenForSession(token):null,sessionExpiresAt:session?.exp||null});});
