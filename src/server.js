...
import { createSession, parseCookies, verifySession, requireAdmin, requireCsrf, csrfTokenForSession, getSessionToken, safeUsernameEqual, verifyAdminPassword, verifyTotp, loginThrottleStatus, recordLoginFailure, clearLoginFailures } from './auth.js';
...
import { totpUri, qrSvg } from './qr.js';
import { isUsableTotpSecret, generateTotpSecret, normalizeTotpSecret } from './totp-secret.js';
...
app.use((req,res,next)=>{
  ...
  res.setHeader('Content-Security-Policy',"default-src 'self'; base-uri 'none'; object-src 'none'; frame-ancestors 'none'; form-action 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; connect-src 'self'");
...
function totpSecretFingerprint(secret=''){
  const normalized=normalizeTotpSecret(secret);
  return crypto.createHash('sha256').update(normalized).digest('hex');
}

async function ensureAdminTotpSecret(){
  if(!config.admin2faEnabled) return {enabled:false,source:'disabled'};
  if(isUsableTotpSecret(config.adminTotpSecret)){
    config.adminTotpSecret=normalizeTotpSecret(config.adminTotpSecret);
    return {enabled:true,source:'env'};
  }
  let stored='';
  const enc=String(await getSetting('admin_2fa_secret_enc','')||'');
  if(enc){
    try{stored=decryptSecret(enc);}catch(e){
      await logError('security','TOTP_SECRET_DECRYPT_FAILED','Stored 2FA secret could not be decrypted; rotating secret',{reason:e.message}).catch(()=>{});
    }
  }
  if(!isUsableTotpSecret(stored)){
    stored=generateTotpSecret(20);
    await setSetting('admin_2fa_secret_enc',encryptSecret(stored));
    await setSetting('admin_2fa_enrolled',false);
    await setSetting('admin_2fa_secret_fingerprint','');
    await logError('security','TOTP_SECRET_GENERATED','Generated new encrypted admin 2FA secret',{source:'database'}).catch(()=>{});
    config.adminTotpSecret=stored;
    return {enabled:true,source:'generated'};
  }
  config.adminTotpSecret=normalizeTotpSecret(stored);
  return {enabled:true,source:'database'};
}
async function admin2faEnrollmentState(){
  if(!config.admin2faEnabled) return {enrolled:false,fingerprint:''};
  const enrolled=Boolean(await getSetting('admin_2fa_enrolled',false));
  const storedFingerprint=String(await getSetting('admin_2fa_secret_fingerprint','')||'');
  const currentFingerprint=totpSecretFingerprint(config.adminTotpSecret);
  return {enrolled:enrolled && Boolean(storedFingerprint) && storedFingerprint===currentFingerprint,fingerprint:currentFingerprint,storedFingerprint};
}

app.post('/api/login',async(req,res)=>{
  const {username='',password='',otp='',setupConfirm=false}=req.body||{};
  const throttle=loginThrottleStatus(req,username);
  if(!throttle.allowed){res.setHeader('Retry-After',String(throttle.retryAfter));return res.status(429).json({ok:false,error:'LOGIN_LOCKED',retryAfter:throttle.retryAfter});}
  const userOk=safeUsernameEqual(username);
  const passOk=userOk ? await verifyAdminPassword(password) : false;
  if(!userOk||!passOk){
    const fail=recordLoginFailure(req,username);
    await logError('security','LOGIN_FAILED','Admin login rejected',{ip:String(req.ip||''),twoFactorEnabled:config.admin2faEnabled,attemptCount:fail.count}).catch(()=>{});
    return res.status(401).json({ok:false,error:'LOGIN_FAILED',requires2fa:config.admin2faEnabled});
  }

  if(config.admin2faEnabled){
    const enrollment=await admin2faEnrollmentState();
    const enrolled=enrollment.enrolled;
    const otpValue=String(otp||'').trim();
    const issuer='LiveChat AI';
    const setupPayload=()=>{
      const uri=totpUri({secret:config.adminTotpSecret,username:config.adminUsername,issuer});
      return {
        ok:false,requires2fa:true,setupRequired:true,
        qrSvg:qrSvg(uri,{size:260,margin:4}),
        manualKey:String(config.adminTotpSecret||'').toUpperCase().replace(/[^A-Z2-7]/g,''),
        issuer,account:config.adminUsername
      };
    };

    // First enrollment is intentionally two-step. Even if a browser/password manager
    // autofills a stale OTP, the first successful password check MUST show the QR.
    if(!enrolled && setupConfirm!==true){
      return res.status(401).json({...setupPayload(),error:'TWO_FACTOR_SETUP_REQUIRED'});
    }

    const otpOk=verifyTotp(otpValue,config.adminTotpSecret);
    if(!otpOk){
      const fail=recordLoginFailure(req,username);
      await logError('security','LOGIN_FAILED','Admin 2FA rejected',{ip:String(req.ip||''),twoFactorEnabled:true,attemptCount:fail.count}).catch(()=>{});
      if(!enrolled){
        return res.status(401).json({...setupPayload(),error:'TWO_FACTOR_SETUP_INVALID'});
      }
      if(!otpValue) return res.status(401).json({ok:false,error:'OTP_REQUIRED',requires2fa:true});
      return res.status(401).json({ok:false,error:'LOGIN_FAILED',requires2fa:true,setupRequired:false});
    }
    if(!enrolled){
      await Promise.all([
        setSetting('admin_2fa_enrolled',true),
        setSetting('admin_2fa_secret_fingerprint',enrollment.fingerprint)
      ]);
      await logError('security','TWO_FACTOR_ENROLLED','Admin 2FA enrollment confirmed',{ip:String(req.ip||'')}).catch(()=>{});
    }
  }

  clearLoginFailures(req,username);
  const token=createSession(config.adminUsername,req);
  const maxAge=Math.max(3600,config.adminSessionHours*3600);
  res.setHeader('Set-Cookie',`lcai_session=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Strict; Max-Age=${maxAge}; Priority=High${config.nodeEnv==='production'?'; Secure':''}`);
  await logError('security','LOGIN_SUCCESS','Admin login success',{ip:String(req.ip||''),twoFactorEnabled:config.admin2faEnabled}).catch(()=>{});
  res.json({ok:true,requires2fa:config.admin2faEnabled});
});
app.post('/api/logout',requireAdmin,(req,res)=>{res.setHeader('Set-Cookie',`lcai_session=; Path=/; HttpOnly; SameSite=Strict; Max-Age=0${config.nodeEnv==='production'?'; Secure':''}`);res.json({ok:true});});
app.get('/api/me',async(req,res)=>{const token=getSessionToken(req);const session=verifySession(token,req);const enrollment=config.admin2faEnabled?await admin2faEnrollmentState():{enrolled:false};const setupRequired=config.admin2faEnabled?!enrollment.enrolled:false;res.json({ok:Boolean(session),user:session?.u||null,twoFactorEnabled:config.admin2faEnabled,twoFactorSetupRequired:setupRequired,csrfToken:session?csrfTokenForSession(token):null,sessionExpiresAt:session?.exp||null});});
...
app.get('/api/conversations',requireAdmin,async(req,res)=>{const r=await pool.query(`SELECT c.chat_id,c.customer_name,c.customer_email,c.status,c.handling_mode,c.takeover_reason,c.takeover_at,c.last_event_at,c.last_member_event_at,c.last_ai_event_at,c.workflow_type,c.workflow_state,c.member_typing,c.member_typing_updated_at,c.created_at,c.updated_at,
  ...
  WHERE c.visible_in_inbox=true AND COALESCE(c.status,'active')<>'closed' AND COALESCE(c.lc_active,true)<>false AND COALESCE(c.lc_routing_status,'') NOT IN ('closed','archived','inactive','deactivated') ORDER BY c.lc_inbox_rank ASC NULLS LAST, COALESCE(c.last_event_at,c.updated_at) DESC LIMIT 200`);res.json({ok:true,items:r.rows});});
...
async function boot(){
  try{
    ...
    const totpState=await ensureAdminTotpSecret();
    if(totpState.enabled) console.log(`Admin 2FA secret ready (${totpState.source})`);
    try{
