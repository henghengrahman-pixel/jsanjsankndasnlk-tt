...
export class LiveChatClient {
  ...
  chatState(summary) {
    const th = summary?.last_thread_summary || summary?.last_thread || (Array.isArray(summary?.threads) ? summary.threads.at(-1) : null) || {};
    const followed = summary?.is_followed;
    const rawStatus = String(summary?.status || th?.status || '').toLowerCase();
    const routingStatus = String(summary?.routing_status || th?.routing_status || '').toLowerCase();
    const terminal = ['closed','archived','inactive','deactivated'].includes(rawStatus)
      || ['closed','archived','inactive','deactivated'].includes(routingStatus)
      || summary?.is_active === false
      || th?.active === false
      || summary?.active === false;
    const active = terminal ? false
      : (typeof th?.active === 'boolean' ? th.active
      : (typeof summary?.active === 'boolean' ? summary.active
      : (typeof summary?.is_active === 'boolean' ? summary.is_active
      : (rawStatus === 'active' ? true : null))));
    return { followed, active, routingStatus, rawStatus, terminal };
  }

  isMyActiveChat(summary) {
    const st = this.chatState(summary);
    // Terminal provider states always win. A stale is_followed=true must never revive
    // a chat LiveChat has already archived/deactivated.
    if (st.terminal || st.active === false) return false;
    // LiveChat's Agent API marks chats followed by the current agent with is_followed.
    // This most closely mirrors the web app's active "My chats" list.
