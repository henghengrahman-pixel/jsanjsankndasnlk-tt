...
export async function getRules(intent){
  ...
  const r=await pool.query(`SELECT category,rule_type,content FROM ai_rules WHERE active=true AND upper(category)=ANY($1::text[]) ORDER BY CASE WHEN upper(category)='GLOBAL' THEN 0 ELSE 1 END,id`,[cats]);
  return r.rows;
}
export async function getKnowledge(intent){
  const cats=sourceCategories(intent);
  const r=await pool.query(`SELECT category,title,content FROM knowledge_base WHERE active=true AND upper(category)=ANY($1::text[]) ORDER BY CASE WHEN upper(category)='GENERAL' THEN 0 WHEN upper(category)='SYNONYM' THEN 1 ELSE 2 END,id LIMIT 120`,[cats]);
  return r.rows;
}
export async function getRelevantKnowledge(query,intent,limit=20){
  ...
  const r=await pool.query(`SELECT id,category,title,content,updated_at FROM knowledge_base WHERE active=true AND upper(category)=ANY($1::text[]) ORDER BY updated_at DESC,id DESC LIMIT 800`,[cats]);
  const scored=r.rows.map(row=>{
    const cat=String(row.category||'').toUpperCase();
    const sem=semanticScore(String(query||''),`${row.title||''} ${cat} ${row.content||''}`);
