...
export async function upsertConversation(chat,{visible=true,state={}}={}){
  ...
  const routing=String(state.routingStatus||'').toLowerCase();
  const terminal=state.active===false || ['closed','archived','inactive','deactivated'].includes(routing);
  const effectiveVisible=Boolean(visible) && !terminal;
  await pool.query(`INSERT INTO conversations(chat_id,customer_name,customer_email,last_event_at,updated_at,visible_in_inbox,lc_is_followed,lc_active,lc_routing_status,lc_summary,lc_inbox_rank,inbox_last_seen_at,status,ended_at)
    VALUES($1,$2,$3,now(),now(),$4,$5,$6,$7,$8::jsonb,$9,CASE WHEN $4 THEN now() ELSE NULL END,CASE WHEN $10 THEN 'closed' ELSE 'active' END,CASE WHEN $10 THEN now() ELSE NULL END)
    ON CONFLICT(chat_id) DO UPDATE SET customer_name=COALESCE(EXCLUDED.customer_name,conversations.customer_name),customer_email=COALESCE(EXCLUDED.customer_email,conversations.customer_email),updated_at=now(),
      visible_in_inbox=CASE WHEN $10 THEN false WHEN conversations.status='closed' AND conversations.ended_at IS NOT NULL AND conversations.ended_at>now()-interval '2 minutes' THEN false ELSE EXCLUDED.visible_in_inbox END,
      lc_is_followed=EXCLUDED.lc_is_followed,lc_active=EXCLUDED.lc_active,lc_routing_status=EXCLUDED.lc_routing_status,lc_summary=EXCLUDED.lc_summary,lc_inbox_rank=CASE WHEN $10 THEN NULL ELSE COALESCE(EXCLUDED.lc_inbox_rank,conversations.lc_inbox_rank) END,
      inbox_last_seen_at=CASE WHEN EXCLUDED.visible_in_inbox AND NOT $10 THEN now() ELSE conversations.inbox_last_seen_at END,
      status=CASE WHEN $10 THEN 'closed' WHEN conversations.status='closed' AND conversations.ended_at IS NOT NULL AND conversations.ended_at<=now()-interval '2 minutes' AND COALESCE(EXCLUDED.lc_active,true)=true THEN 'active' ELSE conversations.status END,
      ended_at=CASE WHEN $10 THEN COALESCE(conversations.ended_at,now()) WHEN conversations.status='closed' AND conversations.ended_at IS NOT NULL AND conversations.ended_at<=now()-interval '2 minutes' AND COALESCE(EXCLUDED.lc_active,true)=true THEN NULL ELSE conversations.ended_at END`,
    [String(chat.id), customer.name||customer.email||null, customer.email||null, effectiveVisible, state.followed ?? null, state.active ?? null, state.routingStatus||null, JSON.stringify(chat||{}), rank, terminal]);
}
...
export async function getRelevantKnowledge(query,intent,limit=14){
  const it=String(intent||'GENERAL').toUpperCase();
  const r=await pool.query(`SELECT id,category,title,content,updated_at FROM knowledge_base WHERE active=true AND (category='GENERAL' OR category=$1) ORDER BY updated_at DESC,id DESC LIMIT 300`,[it]);
  const scored=r.rows.map(row=>{
    const sem=semanticScore(String(query||''),`${row.title||''} ${row.category||''} ${row.content||''}`);
    const categoryBoost=String(row.category||'').toUpperCase()===it?0.22:0;
    return {...row,_score:sem+categoryBoost};
  }).sort((a,b)=>b._score-a._score||new Date(b.updated_at)-new Date(a.updated_at));
  const relevant=scored.filter(x=>x._score>=0.12).slice(0,Math.max(1,Number(limit)||14));
  if(relevant.length) return relevant;
  return r.rows.slice(0,Math.max(1,Math.min(Number(limit)||14,14)));
}
...
export async function tryAcquireProcessLock(name='process_lock'){
  const client=await pool.connect();
  try{
    const r=await client.query('SELECT pg_try_advisory_lock(hashtext($1)) AS locked',[String(name)]);
    if(!r.rows?.[0]?.locked){ client.release(); return null; }
    let released=false;
    return {release:async()=>{ if(released)return; released=true; try{await client.query('SELECT pg_advisory_unlock(hashtext($1))',[String(name)]);}catch{} client.release(); }};
...
export async function getRelevantCanned(query, limit=8, intent='GENERAL'){
  const it=String(intent||'GENERAL').toUpperCase();
  const r=await pool.query(`SELECT source_id,shortcut,title,category,content,tags,scope,source_kind,response_mode,updated_at FROM livechat_canned_responses WHERE active=true ORDER BY CASE WHEN source_kind='manual' THEN 0 ELSE 1 END,updated_at DESC LIMIT 1000`);
  ...
  const scored=r.rows.map(row=>{
    ...
    if(String(row.category||'GENERAL').toUpperCase()===it) score+=2.5;
    if(row.source_kind==='manual') score+=1.5;
    return {...row,_score:score};
  }).sort((a,b)=>b._score-a._score || new Date(b.updated_at)-new Date(a.updated_at));
  const relevant=scored.filter(x=>x._score>=2).slice(0,Math.max(1,Number(limit)||8));
  if(relevant.length) return relevant;
  return scored.filter(x=>x.source_kind==='manual' && ['GENERAL',it].includes(String(x.category||'GENERAL').toUpperCase())).slice(0,Math.max(1,Number(limit)||8));
}
...
export async function saveHumanGuidanceLearning({request,humanAnswer}){
  ...
  const existing=await pool.query(`SELECT id,occurrences FROM learning_examples WHERE source_type='HUMAN_GUIDANCE' AND intent=$1 AND lower(member_text)=lower($2) AND lower(response_text)=lower($3) LIMIT 1`,[intent,memberText,answer]);
  if(existing.rowCount){
    const r=await pool.query(`UPDATE learning_examples SET occurrences=occurrences+1,status='APPROVED',approved_at=COALESCE(approved_at,now()),context_snapshot=$2,updated_at=now() WHERE id=$1 RETURNING *`,[existing.rows[0].id,contextSnapshot]);
    return r.rows[0]||null;
  }
  const r=await pool.query(`INSERT INTO learning_examples(source_type,intent,member_text,response_text,status,occurrences,chat_id,source_event_id,context_snapshot,style_only,approved_at)
...
export async function backfillHumanLearning(limit=5000){
  ...
  let scanned=0,created=0,skipped=0,staleSkipped=0,failed=0;
  const errors=[];
  for(const m of rows.rows){
    scanned++;
    try{
      // Historical CS learning is read-only. It must NOT enforce "current LiveChat
      // session" rules because old closed sessions are valid training examples.
      const r=await captureHumanReplyLearning({chatId:m.chat_id,eventId:m.event_id,responseText:m.text});
      if(r) created++; else skipped++;
    }catch(err){
      const message=String(err?.message||err||'');
      if(String(err?.code||'')==='STALE_SESSION_REQUEST' || /STALE_SESSION_REQUEST/i.test(message)){
        staleSkipped++;
        skipped++;
        continue;
      }
  ...
  return {scanned,created,skipped,staleSkipped,failed,errors};
}
...
// Chats whose latest member message has not received any later AI reply yet.
// Human takeover chats are intentionally excluded: a human operator owns those.
export async function listConversationsNeedingAIReply(limit=20){
  const r=await pool.query(`
    SELECT c.chat_id
    FROM conversations c
    WHERE c.visible_in_inbox=true
      AND COALESCE(c.handling_mode,'AI')<>'HUMAN'
      AND c.last_member_event_at IS NOT NULL
      AND (c.last_ai_event_at IS NULL OR c.last_member_event_at>c.last_ai_event_at)
    ORDER BY c.last_member_event_at ASC
    LIMIT $1`,[Math.max(1,Math.min(Number(limit)||20,100))]);
  return r.rows;
}
