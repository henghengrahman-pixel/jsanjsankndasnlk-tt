...
let csrfToken='';
async function api(url,opt={}){const method=String(opt.method||'GET').toUpperCase();const headers={'Content-Type':'application/json',...(opt.headers||{})};if(!['GET','HEAD','OPTIONS'].includes(method)&&url!=='/api/login'&&csrfToken)headers['X-CSRF-Token']=csrfToken;const r=await fetch(url,{...opt,credentials:'same-origin',headers});const d=await r.json().catch(()=>({}));if(!r.ok){const e=new Error(d.error||`HTTP_${r.status}`);e.code=d.error||'';e.data=d;throw e}return d}
...
async function boot(){const me=await api('/api/me');csrfToken=String(me.csrfToken||'');if(!me.ok){$('#login').classList.remove('hidden');if(me.twoFactorEnabled&&!me.twoFactorSetupRequired){$('#otp').classList.remove('hidden');$('#otpHint').classList.remove('hidden')}return}$('#app').classList.remove('hidden');await Promise.all([loadStatus(),loadChats(),loadRules(),loadKb(),loadCanned(),loadPromos(),loadImportant(),loadHuman(),loadBridge(),loadLearning(),loadLogs()]);startLiveUi()}
let twoFaSetupMode=false;
function showTwoFaSetup(d={}){if(!d.qrSvg)return;twoFaSetupMode=true;$('#twoFaSetup').classList.remove('hidden');$('#twoFaQr').innerHTML=d.qrSvg;$('#twoFaKey').textContent=d.manualKey||'';$('#otp').value='';$('#otp').classList.remove('hidden');$('#otpHint').classList.remove('hidden');$('#otpHint').textContent='Scan barcode terlebih dahulu, lalu masukkan kode 6 digit yang muncul di Authenticator.';$('#loginBtn').textContent='Aktifkan 2FA & Masuk';$('#otp').focus()}
async function submitLogin(){const btn=$('#loginBtn');btn.disabled=true;$('#loginErr').textContent='';try{await api('/api/login',{method:'POST',body:JSON.stringify({username:$('#u').value,password:$('#p').value,otp:$('#otp').value,setupConfirm:twoFaSetupMode})});location.reload()}catch(e){if(e.code==='TWO_FACTOR_SETUP_REQUIRED'||e.code==='TWO_FACTOR_SETUP_INVALID'){showTwoFaSetup(e.data);$('#loginErr').textContent=e.code==='TWO_FACTOR_SETUP_INVALID'?'Kode 2FA belum cocok. Scan barcode yang tampil dan masukkan kode 6 digit terbaru.':'Scan barcode lalu masukkan kode 6 digit.'}else if(e.code==='OTP_REQUIRED'||e.data?.requires2fa){$('#otp').classList.remove('hidden');$('#otpHint').classList.remove('hidden');$('#otp').focus();$('#loginErr').textContent=e.code==='OTP_REQUIRED'?'Masukkan kode Authenticator 6 digit.':'ID, password, atau kode 2FA tidak valid.'}else if(e.code==='LOGIN_LOCKED')$('#loginErr').textContent='Terlalu banyak percobaan login. Coba lagi setelah beberapa menit.';else $('#loginErr').textContent='ID, password, atau kode 2FA tidak valid.'}finally{btn.disabled=false}}
$('#loginBtn').onclick=submitLogin;[$('#u'),$('#p'),$('#otp')].forEach(el=>el?.addEventListener('keydown',e=>{if(e.key==='Enter')submitLogin()}));
