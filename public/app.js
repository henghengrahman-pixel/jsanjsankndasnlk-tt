...
async function boot(){const me=await api('/api/me');csrfToken=String(me.csrfToken||'');if(!me.ok){$('#login').classList.remove('hidden');if(me.twoFactorEnabled){$('#otp').classList.remove('hidden');$('#otpHint').classList.remove('hidden')}return}$('#app').classList.remove('hidden');await Promise.all([loadStatus(),loadChats(),loadRules(),loadKb(),loadCanned(),loadPromos(),loadImportant(),loadHuman(),loadBridge(),loadLearning(),loadLogs()]);startLiveUi()}
async function submitLogin(){const btn=$('#loginBtn');btn.disabled=true;$('#loginErr').textContent='';try{await api('/api/login',{method:'POST',body:JSON.stringify({username:$('#u').value,password:$('#p').value,otp:$('#otp').value})});location.reload()}catch(e){if(e.code==='OTP_REQUIRED'||e.data?.requires2fa){$('#otp').classList.remove('hidden');$('#otpHint').classList.remove('hidden');$('#otp').focus()}if(e.code==='LOGIN_LOCKED')$('#loginErr').textContent='Terlalu banyak percobaan login. Coba lagi setelah beberapa menit.';else if(e.code==='OTP_REQUIRED')$('#loginErr').textContent='Masukkan kode Authenticator 6 digit.';else $('#loginErr').textContent='ID, password, atau kode 2FA tidak valid.'}finally{btn.disabled=false}}
